...
router.post('/report', auth, async (req, res)=>{
    try {
        ...
        var ordersdeliver = await mySqlQury("SELECT COUNT(*) as status_count FROM `tbl_order` WHERE store_id="+store+" AND order_status=4 AND DATE(stutus_change_date) = DATE("+date+")")
        var orderstotalseal = await mySqlQury("SELECT SUM(`gross_total`) as totalsale FROM `tbl_order` WHERE store_id="+store+" AND DATE(order_date) = DATE("+date+")")
        var orderspayment = await mySqlQury("SELECT SUM(`debit_amount`) as expence, SUM(`credit_amount`) as payment FROM `tbl_transections` WHERE store_id="+store+" AND DATE(date) = DATE("+date+")")
...
router.get('/order', auth, async(req, res)=>{
    ...
    const accessdata = await access (req.user)
        ...
        res.render('order_report',{accessdata,storeList,store,Ordersatus,orderdata})
...
router.get('/sales',auth, async(req, res)=>{
    ...
    const accessdata = await access (req.user)
        ...
        res.render('Sales_report',{accessdata,storeList,store,Ordersatus,salesdata})
...
router.get('/expence',auth, async(req, res)=>{
    ...
    const accessdata = await access (req.user)
        ...
        res.render('Expence_report',{accessdata,storeList,store,Ordersatus,Expencedata})
...
router.get('/tax', auth, async(req, res)=>{
    ...
    const accessdata = await access (req.user)
        ...
        res.render('tax_report',{accessdata,storeList,store,taxdata})
