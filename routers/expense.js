    ...
    try {
        ...
        const storeList = await mySqlQury("SELECT id,name FROM tbl_store WHERE status=1 AND delete_flage=0");
            ...
            if(rolldetail[0].roll === "master_Admin"){
                ...
                res.render('expensetype', {type:data, ismulty, storeList,accessdata})
                ...
                res.render('expensetype', {type:data, ismulty:false, storeList:[],accessdata})
...
router.get('/categorylist', auth, async (req, res)=>{
    ...
    const accessdata = await access (req.user)
        ...
        if(rolldetail[0].roll === "master_Admin"){
            ...
            res.render('expenceCategory',{type:data, list:list,ismulty,storeList,accessdata })
            ...
            res.render('expenceCategory',{type:data, list:list,ismulty:false, storeList:[] ,accessdata})
...
router.get('/list', auth, async (req, res)=>{
    try {
        ...
        res.render('expensList',{categ:expencategory, expenlist:expdata, ismulty, storeList,acountlist,accessdata})
