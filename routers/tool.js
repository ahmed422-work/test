        ...
        const rolldetail = await mySqlQury("select id,roll,rollaccess from tbl_roll where id = "+roll+"");
                if(rolldetail[0].roll === "master_Admin"){
                    ...
                    if(multiy[0].type == 1)
                    {
                        ...
                        res.render('roll',{rollList:data,ismulty,storeList,accessdata})
                    }else{
                        var ismulty = false
                        ...
                        res.render('roll',{rollList:data,ismulty:false,storeList:[],accessdata})
                    ...
                    res.render('roll',{rollList:rollList,ismulty,storeList:[],accessdata})
...
router.get('/storesetting', auth, async(req, res)=>{
    ...
    const accessdata = await access (req.user)
        ...
        const rolldetail = await mySqlQury("select id,roll,master from tbl_roll where id = "+roll+"");
            ...
            res.render('storeSetting',{storedata,update,accessdata})
        ...
        const rolldetail = await mySqlQury("select id,roll,master from tbl_roll where id = "+roll+"");
                ...
                if(rolldetail[0].roll === "master_Admin"){
                    ...
                    if(multiy[0].type == 1){
                        ...
                        res.render('storelist',{storeList,accessdata})
...
router.get('/approvedshop/:id', auth, async(req, res)=>{
    ...
    return res.redirect('back')}
        ...
        if(rolldetail[0].roll === "master_Admin"){
            ...
            res.render('store_settings_bymaster',{rolldata,storedata,accessdata})
    ...
    try {
        ...
        const rolldetail = await mySqlQury("select id,roll,master from tbl_roll where id = "+roll+"");
                if(rolldetail[0].roll === "master_Admin" || rolldetail[0].master.includes('write')){
                    const rolldata = await mySqlQury("select * from tbl_roll where delet_flage =0 AND store_ID="+store+"");
                   res.render('shop_add_admin',{rolldata,accessdata})
...
router.get('/staff', auth, async(req, res)=>{
    try {
        ...
        const rolldetail = await mySqlQury("select id,roll,staff from tbl_roll where id = "+roll+"");
                ...
                res.render('staff',{rolldata,staffdata, ismulty ,accessdata})
...
router.get('/setting', auth, async(req, res)=>{
    try {
        ...
        if(rolldetail[0].roll === "master_Admin"){
            ...
            res.render('master_settings',{timezones,masterstore:masterstore[0] ,roll, accessdata})
...
router.post('/setmasterdata',auth,upload.fields([{name: "logo", axCount: 1},{name: "favicon", axCount: 1}]), async (req, res)=>{
    ...
    try {
        ...
        if(rolldetail[0].roll === "master_Admin"){
           ...
           var {multy,store_approved,customer_approved,currency,Symbol_Placement,timezone,printer_pos,storeroll,appname} = req.body;
          ...
          await mySqlQury(`UPDATE tbl_master_shop SET type=${multy},currency_symbol='${currency}',currency_placement=${Symbol_Placement},
...
router.get('/mail',auth, async (req, res)=>{
    try {
        ...
        if( rolldetail[0].mail.includes('read')){
            ...
            res.render('mailsetting',{accessdata,data})
        }
